...
# Baixar um modelo pré-treinado para a língua portuguesa
modelo <- udpipe_download_model(language = "portuguese")

# Carregar o modelo
modelo_udpipe <- udpipe_load_model(file = modelo$file_model)
...
# Anotar o texto
anotacao <- udpipe_annotate(modelo_udpipe, x = texto)
anotacao <- as.data.frame(anotacao)

# Filtrar os adjetivos
adjetivos <- anotacao[anotacao$upos == "ADJ", ]

# Contar os adjetivos
contagem_adjetivos <- table(adjetivos$token)

# Exibir o resultado
print(contagem_adjetivos)


write.table(x = contagem_adjetivos,
            file = "AdjetivosNovaTaquara.txt",  # Save the file as pirates.txt to my desktop
            sep = "\t")  
