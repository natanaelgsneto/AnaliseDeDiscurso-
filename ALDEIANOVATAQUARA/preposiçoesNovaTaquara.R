...
# Lista de preposições em português
preposicoes <- c("a", "ante", "após", "até", "com", "contra", "de", "desde", "em", "entre", 
                 "para", "per", "perante", "por", "sem", "sob", "sobre", "trás")

# Função para mostrar preposições em um texto
mostrar_preposicoes <- function(texto) {
  # Dividir o texto em palavras
  palavras <- unlist(strsplit(texto, "\\W+"))
  # Filtrar palavras que são preposições
  preposicoes_encontradas <- palavras[palavras %in% preposicoes]
  return(preposicoes_encontradas)
}
...
preposicoes_encontradas <- mostrar_preposicoes(texto)
typeof(preposicoes)
l1 = list(preposicoes_encontradas)
typeof(l1)
l1


cat("Preposições encontradas:", preposicoes_encontradas, "\n")


write.table(x = preposicoes_encontradas,
            file = "preposicaoNovaTaquara.txt",  # Save the file as pirates.txt to my desktop
            sep = "\t")  
