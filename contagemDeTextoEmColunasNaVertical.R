...
# Baixe o modelo UDPipe para o português, se necessário
...
# Exemplo de texto
texto <- "Nossas raízes aí assim mostrando que a gente vai falar dessa parte de baixo da árvore, tá nosso estudo. Vai tratar agora da parte de baixo da árvore tem tudo a ver não tem e já foi caindo da terra e se tivéssemos conversado nem isso, pois então é mentira, pô, nós estamos na fase, Ibirapuã tá? É dentro desse sistema de crença, a gente tem três categorias que nós vamos conversar hoje nessa conversa. A gente tá gravando depois a gente vai fazer uma espécie de transcrição e a gente vai fazer uma categorização das falas das narrativas para gerar gráficos para gerar dados isso é pesquisa, né? Diferentemente do projeto de outros projetos de intervenção que não precisa gerar gráfico para assim, não precisa mas a pesquisa a gente termina gerando que esse documentário.
...
# Processamento de texto com UDPipe
anotacoes <- udpipe_annotate(ud_model, x = texto)

# Converta as anotações para um dataframe
anotacoes_df <- as.data.frame(anotacoes)

# Filtre apenas as preposições
preposicoes <- anotacoes_df %>% filter(upos == "ADP")

# Contar as preposições
contagem_preposicoes <- preposicoes %>%
  count(token) %>%
  spread(token, n, fill = 0)

# Exibir a contagem de preposições
print(contagem_preposicoes)
t(contagem_preposicoes)
